...
function handleAttachment(item) {
  ...
  function handlePreviousClick(currentPage) {
    ...
    const url = `http://localhost:3001/getInboxMsgs?page=${haha}`;
  ...
  const initialPage = 1;
  const url = `http://localhost:3001/getInboxMsgs?page=${initialPage}`;
    ...
    {data.map((item, index) => (
      <li key={index} onClick={() => handleCardClick(item)} class="email-card">
        ...
        <div class="email-content">
          <strong class="email-subject">{item.subject ? item.subject : "No Subject"}
</strong>
